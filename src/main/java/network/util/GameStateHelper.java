...
public final class GameStateHelper {
    ...
    public static GameState createGameState(final Position[] snakePosition,
        ...
        final Boolean[] states = mergeStates(
                currentDirection == Direction.DOWN
                        ? getNegativeStates()
                        : getStatsForDirectionUp(snakeBodyPosition, headPosition),
                currentDirection == Direction.LEFT
                        ? getNegativeStates()
                        : getStatsForDirectionRight(snakeBodyPosition, headPosition),
                currentDirection == Direction.UP
                        ? getNegativeStates()
                        : getStatsForDirectionDown(snakeBodyPosition, headPosition),
                currentDirection == Direction.RIGHT
                        ? getNegativeStates()
                        : getStatsForDirectionLeft(snakeBodyPosition, headPosition),
    ...
    public static double getScoreForAction(final Action action,
        ...
        switch (action) {
            case MOVE_UP:
                ...
                score += foodPosition
                        .equals(new Position(headPosition.getX(), headPosition.getY() - GameUtils.PLAYER_SIZE))
                                ? FOOD_EATEN_REWARD
                                : 0;
                break;
            case MOVE_RIGHT:
                ...
                score += foodPosition
                        .equals(new Position(headPosition.getX() + GameUtils.PLAYER_SIZE, headPosition.getY()))
                                ? FOOD_EATEN_REWARD
                                : 0;
                break;
            case MOVE_DOWN:
    ...
    /**
     * Get number of possible states. There are 4 directions in which snake can see.
     * Number of inputs is equal to
    ...
    /**
     * Position counts as positive if it is inside the bounds and if it is not
     * already contained within snake body
     * positions.
     *
